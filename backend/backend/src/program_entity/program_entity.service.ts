...
import { program_entity, sections } from 'models_curriculum';
...
export class ProgramEntityService {
  ...
  async findOneCurr(id: number) {
    try {
      const result = await this.sequelize.query(
        `SELECT * FROM curriculum.program_entity JOIN curriculum.program_entity_description ON program_entity.prog_entity_id = program_entity_description.pred_prog_entity_id WHERE prog_entity_id = ${id}`,
      );
      return result[0];
    } catch (error) {
      return error.message;
  ...
  async updateStatus(id: number, fields:any) {
    try {
      const result = await program_entity.update(
        {
          prog_status: fields.status
        },
        {
          where: {
            prog_entity_id: id,
  ...
  async findAllTabe(id: number) {
    try {
      ...
      LEFT JOIN curriculum.program_entity_description ON program_entity.prog_entity_id = program_entity_description.pred_prog_entity_id
      LEFT JOIN curriculum.sections ON program_entity.prog_entity_id = sections.sect_prog_entity_id
      LEFT JOIN curriculum.section_detail ON sections.sect_id = section_detail.secd_sect_id
      LEFT JOIN curriculum.section_detail_material ON section_detail.secd_id = section_detail_material.sedm_secd_id
  ...
  async updateSection(id: number, updateProgramEntityDto: any) {
    try {
      const data = {
        sect_title: updateProgramEntityDto.sect_title,
        description: updateProgramEntityDto.description,
      };
      const paramProgram = `[${JSON.stringify(data)}]`;
      const result = await this.sequelize.query(
        `CALL curriculum.update_section_fix(${id},'${paramProgram}')`,
      );
      return { data: result, message: 'Sukses', status: 200 };
    } catch (error) {
  ...
  //       sect_descrption :section.description
  //     }
  //     const paramProgram = `[${JSON.stringify(data)}]`;
